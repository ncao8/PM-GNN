...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        Din=dataset.num_node_features
        Dout_pre=128
        Dedge_pre=Din*Dout_pre
        Dedge=Dout_pre*Dout_pre

        num_edge_features=3

        self.nn_pre = torch.nn.Sequential(
                torch.nn.Linear(num_edge_features,64),
                torch.nn.ReLU(),
                torch.nn.Linear(64,64),
                torch.nn.ReLU(),
                torch.nn.Linear(64,Dedge_pre))
        ...
        self.nn = torch.nn.Sequential(
                ...
                torch.nn.Linear(64,64),
                torch.nn.ReLU(),
                torch.nn.Linear(64,Dedge))

        self.conv_pre = NNConv(Din, Dout_pre, self.nn_pre)
        self.conv1 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv2 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv3 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv4 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv5 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv6 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv7 = NNConv(Dout_pre,Dout_pre, self.nn)
        self.conv8 = NNConv(Dout_pre,Dout_pre, self.nn)

        self.lin1 = torch.nn.Linear(2*Dout_pre, 128)
    ...
    def forward(self, data):
        ...
        x = self.conv_pre(x, edge_index, edge_attr1)
        x = F.relu(x)
        x = self.conv1(x, edge_index, edge_attr2)
        x = F.relu(x)
        x = self.conv2(x, edge_index, edge_attr1)
        x = F.relu(x)
        x = self.conv3(x, edge_index, edge_attr2)
